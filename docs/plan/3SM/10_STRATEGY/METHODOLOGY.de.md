...
## Regulatorischer Rahmen (Hintergrund)
...
- **Designkonsequenz — ausschliesslich Produktdaten (MASTER D27):** jeder Datenbankeintrag führt: HS/CN-Code (inferiert) und Herkunft (CH/EU/Drittland). Keine Rechtskategorie und kein Verbots-Betroffenheits-Flag werden gespeichert — die rechtskonform/unrechtmässig-Sicht und die Schweizer-Verbotsrelevanz sind analytische Folgerungen dieser Dokumentation (LEAD_SDS.md), nie Datenbankfelder. Dass die SDB-Methode Gesamt-Pb nicht messen kann, gilt unverändert.
