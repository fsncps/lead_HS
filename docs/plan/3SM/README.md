...
- **Design** (later) answers *how exactly*: the technical shape of the
  database and the data collection.
- **Implementation** (later) contains the finished step-by-step work plans.

Only the strategy stage exists so far. These are working documents: they are
meant to be corrected and refined, and ordinary edits never silently advance
...
when the project reaches those stages.
