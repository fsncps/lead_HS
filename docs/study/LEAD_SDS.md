---
updated: 2026-09-18
---

# Lead identification via SDS — compounds, legal status, feasibility

## Abstract

This document collects what is known about lead in paints and where the law
stands. Lead compounds appear in paints as colour pigments (e.g. lead
chromate yellows/reds, white lead), rust-protection additives (red lead) and
drying agents in solvent-based alkyd paints (lead octoate). EU law bans
several of them in paints or for consumer use, but rust-protection additives
and drying agents remain legal; Switzerland bans all paints with ≥ 0.01%
total lead (100 ppm) via ChemRRV Anhang 2.8, shielded from EU-lawful imports
by the VIPaV exceptions catalogue (Cassis-de-Dijon context). Safety data
sheets must list classified hazardous ingredients above 0.1% — so they
reveal deliberately added lead, but not the 100–1000 ppm band that matters
for the Swiss ban. The document also lists the first hard EU-market
presence findings and prior studies that calibrate expectations.

## DECISIONS (condensed; authoritative list in MASTER.md)

- Dictionary v0.1 scope: pigments, anticorrosives, driers, artists'
  pigments; unverified CAS flagged (*) until ECHA EC-inventory check.
- SDS Section 3 as primary detection source; UFI as join key; Section 15
  authorisation statements as anomaly signal (MASTER D5).
- Feasibility verdict: declared classified lead ≥ 0.1% is visible; the
  100–1000 ppm band is invisible — headline caveat (MASTER D15).

## OPEN ITEMS

- CAS verification: lead naphthenate (61790-14-5?), lead neodecanoate
  (27253-29-8?) vs ECHA EC inventory — before dictionary freeze.
- OJ reference of the 17 Mar 2022 lead-chromate refusal.
- Artists' colours: member-state permit practice (ILO C13) NL/IT/DE/FR.
- FR/IT red-lead primer retail sweep ("minium de plomb"/"minio rosso").

## Lead-compound dictionary (v0.1)

| CAS | Name / synonym | C.I. | Function | Segments | EU legal status | SDS-visible |
|---|---|---|---|---|---|---|
| 7758-97-6 | Lead chromate (chrome yellow) | PY34 family | pigment | industrial, road-marking | Annex XIV #10, sunset 2015-05-21; **all authorisations refused 17 Mar 2022** | ≥0.1% |
| 1344-37-2 | Lead sulfochromate yellow | PY34 (C.I. 77603) | pigment | enamels | Annex XIV #11; authorisations annulled 2019 / refused 2022 | ≥0.1% |
| 12656-85-8 | Lead chromate molybdate sulfate red | PR104 (C.I. 77605) | pigment | industrial topcoats | Annex XIV #12; authorisations refused 2022 | ≥0.1% |
| 1314-41-6 | Red lead / minium (Pb₃O₄) | PR105 | anticorrosive | marine/steel primers | **no EU restriction; documented EU niche presence (WERDOL DE, BRAVA SE)** | ≥0.1% (Repr 1A route) |
| 1317-36-8 | Litharge (PbO) | 77577 | intermediate | drier precursor | no restriction | ≥0.1% |
| 1319-46-6 | White lead (basic lead carbonate) | PW1 (77597) | pigment | **artists' oil colours**, art/restoration | **Annex XVII #16: banned in paints**; ILO C13 member-state permit for restoration; artists' colours sold under national regimes (NL/IT documented) | ≥0.1% |
| 598-63-0 | Lead carbonate | — | pigment (historic) | — | Annex XVII #16 | n/a |
| 7446-14-2 / 15739-80-7 | Lead sulfate / basic lead sulfate | 77630 | extender, primers | anticorrosive | **Annex XVII #17: banned in paints** | n/a |
| 301-08-6 | Lead octoate (bis(2-ethylhexanoate)) | — | **drier** | alkyds (3208) | no restriction verified; **EU-market presence unverified — key open stream** | ≥0.1% |
| 61790-14-5* | Lead naphthenate | — | drier | alkyds/inks | unverified | ≥0.1% |
| 27253-29-8* | Lead neodecanoate | — | drier | industrial alkyds | unverified | ≥0.1% |
| (PY41) | Lead antimonate (Naples yellow) | PY41 | pigment | artists' colours | **not a carbonate/sulfate → outside entries 16/17**; openly sold (NL/IT) | ≥0.1% |
| (Pb₂SnO₄) | Lead-tin yellow (giallorino) | — | pigment | artists' colours, restoration | outside entries 16/17; openly sold (IT) | ≥0.1% |

(*) CAS to verify against ECHA EC inventory before dictionary freeze.
Related: non-lead chromate anticorrosives (strontium chromate etc.) are also
Annex XIV (sunset 2019) — screening flag when present.

## EU legal frame (verified)

- Annex XVII 16/17: lead carbonates/sulfates banned **in paints** ("where
  the substance or mixture is intended for use as paint"); member states may
  permit use/placing on the market for **restoration and maintenance of
  works of art and historic buildings** (ILO Convention 13) — the hook under
  which artists' lead colours survive; practice varies by country.
- Annex XIV 10–12 + entries 28–30: lead chromates post-sunset (2015);
  DCC/DCL authorisations annulled (GC T-837/16, 2019), effects maintained
  until fresh decision (CJEU C-389/19 P, 2021), **all 8 uses refused
  (Commission decision announced 17 Mar 2022)** → no lawful EU supply since;
  OJ reference still to be pulled.
- Entry 63: lead ≥0.05% in mouthable consumer **articles** (paint is a
  mixture → context); PVC amendment 2023/1422 unverified.
- SDS cutoffs (Reg (EU) 2020/878, Annex II REACH): CMR 1A/1B ≥ 0.1% must be
  listed in Sec. 3; OEL-listed substances must be listed (no explicit % for
  that route); ranges permitted. SDS free of charge on request (Art. 31(8)).

## Swiss layer (VIPaV / ChemRRV / Cassis de Dijon)

- **Cassis de Dijon (CH — autonomous, not bilateral):** EU/EEA-lawful
  products may enter the Swiss market without Swiss re-approval (THG
  Art. 16a, autonomously adopted 1 July 2010; MRA = separate THG
  instrument, out of scope); exceptions (Federal Council, THG Art. 16a
  Abs. 2 lit. e i.V.m. Art. 4 Abs. 3–4) catalogued in **VIPaV
  (SR 946.513.8) Art. 2**; statutory list basis THG Art. 31 Abs. 2.
- **VIPaV Art. 2 Bst. a Ziff. 1** = "bleihaltige Anstrichfarben und Lacke
  sowie damit behandelte Produkte (Anhang 2.8 ChemRRV)" — the first item of
  the exceptions catalogue, in force since 2010, still in the SECO
  Negativliste (1 Jan 2026); enforcement: BAFU. (SECO e-mail's "Art 2a(1)"
  read as this item — no Art. 2a exists; inference.) **Art. 16 VIPaV** =
  list-maintenance provision (context only).
- **ChemRRV Anhang 2.8** (2005 wording; current text OPEN): lead paint =
  total Pb ≥ 0.01% (100 ppm); ban on placing on the market incl. treated
  articles; no standing lead exceptions (transitions expired 2006/07).
  Stricter than the EU — hence the exception.
- Five-yearly review of the **entire** exception catalogue (VIPaV Art. 3),
  conducted by SECO: 2023 outcome "keep" (Eignung/Erforderlichkeit/
  Verhältnismässigkeit affirmed); next ~2028. Governance chain: the
  requesting/owning office (BBL per commissioning context 2026-09-10 —
  verify before naming in deliverables) is responsible for implementation,
  monitoring, revision; BAFU enforces; the study feeds this cycle with a
  document-based decision basis, outcome-neutral.
- Chemicals/CdD boundary: a product enters either via Swiss chemicals law
  or via CdD, not mixed; follow-up duties survive (Anmeldestelle Chemikalien).
- ChemO/REACH-alignment details beyond this frame remain an open dossier
  item (see the strategy MASTER open items: [docs/plan/3SM/10_STRATEGY/MASTER.md](../plan/3SM/10_STRATEGY/MASTER.md)).

## EU-market presence — first hard findings (2026-08-31)

| Stream | Status | Evidence (seed records) |
|---|---|---|
| Lead chromate pigments | **No lawful EU supply since 17 Mar 2022** | court records; ECHA register |
| Red lead primers | **Present (niche: marine/heritage)** | Epifanes WERDOL Bleimennige, DE marine retail, SDS v7 12.01.2021; BRAVA blymönja SE (professional-only, permit); NL/DE DIY substituted lead-free |
| Lead driers in alkyds | **Unknown** — neither presence nor absence documented | first target of the SDS sampling pilot |
| Artists' oil colours (3213) | **Present** | Old Holland Cremnitz White No. 3 (PW1, NL); Zecchi biacca/giallorino/minio/PY41 (IT); Michael Harding (UK→EU, unverified) |
| Road paints (legacy) | 63% of 236 samples >10 mg/kg Pb, up to 17.2 wt%, 11 countries | Turner & Filella 2022 (DOI 10.1016/j.envpol.2022.120492); provenance unresolved |

## Feasibility assessment

- **Visible:** intentionally added classified lead compounds ≥ 0.1% (pigments at % levels; driers typically ≥0.1% Pb — flagged as domain inference).
- **Invisible — headline caveat:** the Swiss ban engages at **100 ppm total
  Pb**, the EU SDS floor is **1000 ppm (0.1%) for classified compounds**: a
  paint can be EU-lawful, fully documented, and still exceed the Swiss ban
  without the sheet showing it. Plus the usual blind spots: impurity lead,
  under-declaring, stale sheets (NO 2014: 320 SDS generic/incomplete;
  artificial-stone 2022: constituents under-reported).
- **No laboratory in this project:** blind spots cannot be bounded
  experimentally. Mitigation is document-only corroboration (see
  `METHODOLOGY.md`): cross-check positives against TDS, labels, retailer
  listings, older SDS versions, cross-market brand variants; report
  contradictions as findings; carry an explicit limitations section in the
  final discussion basis.
- **Availability:** manufacturer/brand sites + B2B portals (scrape; DE/FR/IT/EN); no public EU SDS repository; PCN submissions not public; UFI in Sec. 1.1 as join key.
- **CN assignment from SDS:** medium (aqueous?) + binder (acrylic/vinyl/polyester/other) → CN8; borderline: ≥50%-solvent polymer solutions (3208 10/20 vs 90), hybrids. Artists' colours → 3213 (census annex).

## Prior studies (calibration)

- **No EU/EEA-wide or Swiss lead-in-paint market survey exists** — the gap this study fills (the EU-market stream findings above are the first seed records).
- Mexico 2026 (213 paints, lab): 55% >90 ppm; 91% of leaded ≈ lead chromate (Pb:Cr 4:1); 12 leaded products labelled "no lead". DOI 10.1093/annweh/wxag023
- IPEN global campaign (59 countries, >5,000 paints, lab): high prevalence in developing markets; country database Oct 2024. ipen.org/our-work/lead-paint
- Kumar et al. 2018 review: often >10,000 mg/kg in LMIC paints. DOI 10.1016/j.envint.2018.08.052
- Turner & Filella 2022: European road paints (above).
- WHO 2024: 48% of countries have binding lead-paint controls.
- Working hypothesis (to test): Swiss consumer decorative ≈ 0% declared lead; niche documented presence in marine/anticorrosive (3208) and artists' colours (3213); third-country imports higher; lead driers unknown.

## REFERENCES

Legal EU: consolidated REACH CELEX 02006R1907-20221014; Reg (EU) 125/2012; Reg (EU) 2020/878; Reg (EU) 2015/628; GC T-837/16 + CJEU C-389/19 P — via EUR-Lex/ICLR + Wayback snapshots (accessed 2026-08-31). Legal CH: THG Art. 16a (archived 2010), VIPaV Art. 1–2 (archived 2010), ChemRRV Anh. 2.8 (archived 2005), SECO Negativliste 2026, SECO 2023 five-yearly review, Anmeldestelle Chemikalien (URLs in METHODOLOGY.md). Market: product/SDS URLs in METHODOLOGY.md §REFERENCES. Studies: DOIs and OpenAlex records as cited. SDS quality: DOI 10.2478/s13382-014-0302-8; DOI 10.1093/annweh/wxac020; DOI 10.1002/ajim.20613.
