...
- **Conséquence de conception — données produit uniquement (MASTER D27) :** chaque enregistrement de la base porte : code SH/CN (inféré) et provenance (CH/UE/pays tiers). Aucun champ « catégorie juridique » ni aucun indicateur d'application de l'interdiction n'est stocké — la lecture licite/illicite et la pertinence au regard de l'interdiction suisse sont des conclusions analytiques de cette documentation (LEAD_SDS.md), jamais des champs de base de données. La méthode FDS ne peut toujours pas mesurer le plomb total.
